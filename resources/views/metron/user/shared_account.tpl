...
<html lang="en">
    <head>
        <title>A Shared account &mdash; {$config["appName"]}</title>
                                                    ...
                                                    <div class="kanban-container" style="width: 750px;">
                                                        ...
                                                        <div data-id="_inprocess" data-order="1" class="kanban-board {if $typeNum===1}kanban-1{elseif $typeNum===2}kanban-2{elseif $typeNum===3}kanban-3{elseif $typeNum===4}kanban-4{elseif $typeNum >= 5}kanban-5{/if} " style="width: 250px; margin-left: 0px; margin-right: 0px;">
                                                            <header class="kanban-board-header light-{$css[$fornum]}">
                                                                <div class="kanban-title-board">{$acc['name']}&nbsp;&nbsp;&nbsp;{if $acc['class'] > 0}{$metron['user_level'][$acc['class']]}exclusive{/if}
                                                                    {if $user->class >= $acc['class'] && $acc['check'] === true}
                                                                    <span class="btn btn-primary font-weight-bold btn-sm pull-r cursor_onclick" onclick="shared_account.check('{$type@key}', '{$acc@key}');">Test account</span>
                                                                    {/if}
                                                                </div>
                                                            </header>
                                                            <main class="kanban-drag">
                                                                ...
                                                                <div class="{$style[$theme_style]['shared']['item']} kanban-item copy-text cursor_onclick" data-clipboard-text="{$acc['passwd']}">{if $acc['hidepawd'] === true}Click on the copy in the password box after paste{else}{$acc['passwd']}{/if}</div>
                                                                {else}
                                                                <label>account：</label>
                                                                <div class="{$style[$theme_style]['shared']['item']} kanban-item">The account only <span class="text-{$css[$fornum]}">{$metron['user_level'][$acc['class']]}[LV.{$acc['class']}]</span> And more members are available</div>
                                                                <label>password：</label>
                                                                <div class="{$style[$theme_style]['shared']['item']} kanban-item">The account only <span class="text-{$css[$fornum]}">{$metron['user_level'][$acc['class']]}[LV.{$acc['class']}]</span> And more members are available</div>
                                                                {/if}
                                                            </main>
